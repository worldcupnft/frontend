  ...
  function isotopInit() {
    if ($.exists('.cs-isotop')) {
      var $grid = $('.cs-isotop').isotope({
        ...
        filter: function () {
          return qsRegex ? $(this).attr('data-title').match(qsRegex) : true;
        }
      });
      ...
      var $quicksearch = $('.filter-item').keyup(debounce(function () {
      ...
      $('.cs-filter_head_left .cs-clear_btn').click(function () {
        $(".filter-item").val("");
        $(".filter-item").keyup();
      })
    }
